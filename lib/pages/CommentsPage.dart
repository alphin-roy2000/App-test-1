...
class CommentsPageState extends State<CommentsPage> {
  ...
  retrieveComments(){
    return StreamBuilder(
      stream: commentsReference.document(postId).collection("comment").orderBy("timestamp",descending: false).snapshots(),
  ...
  saveComment(){
    commentsReference.document(postId).collection("comment").add({
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(children: <Widget>[Expanded(child: retrieveComments()),
      Divider(),
      ListTile(
        title: TextFormField(controller: commentTextEditingController,
        ...
        trailing: OutlineButton(onPressed: saveComment,
  ...
  Widget build(BuildContext context) {
    ...
    child: Container(
      ...
      child: Column(children: <Widget>[
        ListTile(
           title: Text(username +":  "+comment,style: TextStyle(color: Colors.black,fontSize:18.0),
